...
void RustGenerator::initRustKeywords()
{
    ...
    const char *keywords[] = { "as",      "break",  "const",  "continue", "crate",    "else", "enum",     "extern",
...
string RustGenerator::toSnakeCase(const string &name)
{
    ...
    for (size_t i = 0; i < name.length(); ++i)
    {
        ...
        if (isupper(c))
        {
            ...
            // lowercase
            if (i > 0 && (!previousWasUpper || (i + 1 < name.length() && islower(name[i + 1]))))
            {
                result += '_';
            }
            result += tolower(c);
        ...
        case DataType::data_type_t::kArrayType:
            {
                ...
                if (arrayType)
                {
                    return "[" + getTypeString(arrayType->getElementType()) + "::default(); " +
                           to_string(arrayType->getElementCount()) + "]";
                }
                return "Default::default()";
            }
...
void RustGenerator::generateOutputFiles(const string &fileNameExtension)
{
    ...
    generateOutputFile(rustFileName + ".rs", "rust_template", m_templateData, kRustTemplate);
}
...
cpptempl::data_map RustGenerator::getFunctionTemplateData(Group *group, Function *fn)
{
    ...
    determineReturnTypes(fn, hasOutParams, outParamTypes, rustReturnType, clientReturnType);
    ...
    bool hasReturnValue = funcReturnType && funcReturnType->getDataType() != DataType::data_type_t::kVoidType;
    ...
    {
        serverHandlerCode = generateRegularServerHandler(fn, codecParamName, sequenceParamName);
    }
    ...
    string clientMethodCode = generateClientMethodCode(fn, clientOutParamTypes, clientHasOutParams);
...
cpptempl::data_map RustGenerator::getFunctionTypeTemplateData(Group *group, FunctionType *fn)
{
    ...
    if (returnType && returnType->getDataType() != DataType::data_type_t::kVoidType)
    {
...
void RustGenerator::setTemplateComments(Symbol *symbol, cpptempl::data_map &symbolInfo)
{
...
string RustGenerator::getFunctionPrototype(Group *group, FunctionBase *fn, const string &interfaceName,
                                           const string &name, bool insideInterfaceCall)
{
    ...
    for (auto param : fn->getParameters().getMembers())
    {
        ...
        if (dir == param_direction_t::kOutDirection || dir == param_direction_t::kInoutDirection)
        {
            prototype << "&mut ";
...
string RustGenerator::getTypeString(DataType *dataType)
{
    ...
    switch (dataType->getDataType())
    {
        ...
        {
            ArrayType *arrayType = dynamic_cast<ArrayType *>(dataType);
            return format_string("[%s; %d]", getTypeString(arrayType->getElementType()).c_str(),
                                 arrayType->getElementCount());
        }
        case DataType::data_type_t::kListType:
        {
            ListType *listType = dynamic_cast<ListType *>(dataType);
            return format_string("Vec<%s>", getTypeString(listType->getElementType()).c_str());
        }
        case DataType::data_type_t::kStructType:
        case DataType::data_type_t::kEnumType:
        case DataType::data_type_t::kUnionType:
        ...
        case DataType::data_type_t::kFunctionType:
        {
            ...
            return format_string("fn(%s) -> %s", "/* params */",
...
{
    return dataType && (dataType->getDataType() == DataType::data_type_t::kListType ||
...
bool RustGenerator::isStringType(DataType *dataType)
{
    ...
    {
        BuiltinType *builtinType = dynamic_cast<BuiltinType *>(dataType);
        return builtinType->getBuiltinType() == BuiltinType::builtin_type_t::kStringType;
    }
...
param_direction_t RustGenerator::getDirection(StructMember *param)
{
    ...
    if (direction != param_direction_t::kInDirection && direction != param_direction_t::kOutDirection &&
...
std::string RustGenerator::generateParamWrite(StructMember *param, const std::string &paramName)
{
    ...
    switch (dataType->getDataType())
    {
        ...
        {
            BuiltinType *builtinType = dynamic_cast<BuiltinType *>(dataType);
            return generateBuiltinTypeWrite(builtinType, "request_codec", paramName, false);
        }
        case DataType::data_type_t::kStructType:
        case DataType::data_type_t::kEnumType:
        ...
        {
            ListType *listType = dynamic_cast<ListType *>(dataType);
            return string("request_codec.start_write_list(") + paramName + ".len() as u32)?;\n" +
        ...
        {
            AliasType *aliasType = dynamic_cast<AliasType *>(dataType);
            return generateParamWriteForDataType(aliasType->getElementType(), paramName);
        }
        default:
...
std::string RustGenerator::generateTypeRead(DataType *dataType, const std::string &codecName)
{
    if (!dataType)
        return "()";
    ...
    switch (dataType->getDataType())
    {
        ...
        case DataType::data_type_t::kArrayType:
        {
            ...
            uint32_t elementCount = arrayType->getElementCount();
            string mutCodecName = codecName;
            // Only add &mut for response_codec, not for codec parameter which is
            // already &mut
            if (mutCodecName == "response_codec")
            ...
            string elementRead = generateTypeRead(elementType, mutCodecName);
            return string("(|| -> ErpcResult<[_; ") + to_string(elementCount) + "]> {\n" +
                   "                        let mut array = [" + getTypeString(elementType) + "::default(); " + to_string(elementCount) + "];\n" +
        ...
        case DataType::data_type_t::kListType:
        {
            ...
            string elementRead = generateTypeRead(listType->getElementType(), codecName);
...
std::string RustGenerator::generateListItemWrite(DataType *elementType, const std::string &itemName)
{
    return generateParamWriteForDataType(elementType, itemName);
}

std::string RustGenerator::generateParamWriteForDataType(DataType *dataType, const std::string &paramName)
{
    if (!dataType)
        return "/* unknown type */";
    ...
    switch (dataType->getDataType())
    {
        case DataType::data_type_t::kBuiltinType:
        {
            ...
            if (builtinType->getBuiltinType() == BuiltinType::builtin_type_t::kStringType ||
                builtinType->getBuiltinType() == BuiltinType::builtin_type_t::kBinaryType)
            {
                actualParam = paramName; // Don't dereference for strings/binary
            }

            return generateBuiltinTypeWrite(builtinType, "request_codec", actualParam, false);
        }
        case DataType::data_type_t::kStructType:
        case DataType::data_type_t::kEnumType:
            return paramName + ".write(&mut request_codec)";
...
std::string RustGenerator::generateMemberWrite(StructMember *member, const std::string &memberName)
{
...
std::string RustGenerator::generateClientRequestSerialization(Function *fn, const std::vector<StructMember *> &inParams)
{
    std::ostringstream requestCode;

    Interface *interface = fn->getInterface();
    string interfaceServiceId = "ServiceId::" + interface->getName();
    string interfaceMethodId = interface->getName() + "Method::" + toPascalCase(fn->getName());
...
std::string RustGenerator::generateClientResponseDeserialization(Function *fn,
                                                                 const std::vector<DataType *> &outParamTypes,
                                                                 const std::vector<std::string> &outParamNames,
                                                                 bool hasOutParams)
{
    std::ostringstream responseCode;

    DataType *returnType = fn->getReturnType();
    bool isVoidReturn = !returnType || returnType->getDataType() == DataType::data_type_t::kVoidType;

    // For oneway methods, no response is expected
    if (fn->isOneway())
    {
        return "            Ok(())";
    }

    // For void return types without out params, no response deserialization needed
    if (isVoidReturn && !hasOutParams)
    {
        return "            Ok(())";
    }
    ...
    for (auto param : fn->getParameters().getMembers())
    {
        std::string rustName = escapeKeyword(toSnakeCase(param->getName()));
        rustToOriginalParamNames[rustName] = param->getName();
        if (lengthParams.find(param->getName()) != lengthParams.end())
        {
            rustLengthParams.insert(rustName);
        }
    }

    auto writeLengthAssignments = [&](std::ostringstream &stream) {
        for (const auto &paramName : outParamNames)
        {
            if (rustLengthParams.find(paramName) == rustLengthParams.end())
            {
                continue;
            }

            std::string originalName = paramName;
            auto originalIt = rustToOriginalParamNames.find(paramName);
            if (originalIt != rustToOriginalParamNames.end())
            {
                originalName = originalIt->second;
            }

            StructMember *referencedFrom =
                findParamReferencedFromAnn(fn->getParameters().getMembers(), originalName, LENGTH_ANNOTATION);
            if (referencedFrom)
            {
                std::string sourceName = escapeKeyword(toSnakeCase(referencedFrom->getName()));
                stream << "            let " << paramName << " = " << sourceName << ".len() as u32;\n";
            }
        }
    };

    if (hasOutParams && returnType && returnType->getDataType() != DataType::data_type_t::kVoidType)
    {
        ...
        for (size_t i = 0; i < outParamTypes.size(); ++i)
        {
            ...
            if (rustLengthParams.find(paramName) != rustLengthParams.end())
            {
                // This is a @length parameter - derive its value from the associated data
                responseCode << "            // Note: " << paramName
                             << " is derived from the associated data length\n";
                continue;
            }
        ...
        responseCode << "            let return_value = " << generateTypeRead(returnType, "&mut response_codec")
                     << "?;\n";

        writeLengthAssignments(responseCode);
    ...
    else if (hasOutParams)
    {
        ...
        if (outParamTypes.size() == 1)
        {
            ...
            if (rustLengthParams.find(paramName) == rustLengthParams.end())
            {
                responseCode << generateOutParamRead(outParamTypes[0], outParamNames[0]);
            }
    ...
    else if (returnType && returnType->getDataType() != DataType::data_type_t::kVoidType)
    {
...
std::string RustGenerator::generateOutParamRead(DataType *paramType, const std::string &paramName)
{
...
std::string RustGenerator::getCodecNameForType(DataType *dataType, const std::string &baseCodecName)
{
...
std::string RustGenerator::generateServerParameterDeserialization(Function *fn, const std::string &codecParamName)
{
    stringstream code;
    ...
    for (auto param : inParams)
    {
        ...
        code << "                    // Deserialize " << paramName << " parameter\n";

        if (isStringType(paramType))
        {
            code << "                    let " << paramName << " = " << codecParamName << ".read_string()?;\n";
        }
        else if (paramType->getDataType() == DataType::data_type_t::kAliasType)
        {
            ...
            else if (underlyingType->getDataType() == DataType::data_type_t::kBuiltinType)
            {
                ...
                switch (builtinType->getBuiltinType())
                {
                    ...
                    case BuiltinType::builtin_type_t::kBinaryType:
                        ...
                        break;
                    default:
                        code << "                    let " << paramName << " = Default::default(); // TODO: Handle "
                             << builtinType->getName() << "\n";
                        break;
                }
            }
            else
            {
                ...
                code << "                    let " << paramName << " = " << typeName << "::read(codec)?;\n";
            }
        }
        else if (paramType->getDataType() == DataType::data_type_t::kBuiltinType)
        {
            BuiltinType *builtinType = dynamic_cast<BuiltinType *>(paramType);
            switch (builtinType->getBuiltinType())
            {
                case BuiltinType::builtin_type_t::kUInt8Type:
                    code << "                    let " << paramName << " = " << codecParamName << ".read_uint8()?;\n";
                    break;
                case BuiltinType::builtin_type_t::kUInt16Type:
                    code << "                    let " << paramName << " = " << codecParamName << ".read_uint16()?;\n";
                    break;
                case BuiltinType::builtin_type_t::kUInt32Type:
                    code << "                    let " << paramName << " = " << codecParamName << ".read_uint32()?;\n";
                    break;
                case BuiltinType::builtin_type_t::kUInt64Type:
                    code << "                    let " << paramName << " = " << codecParamName << ".read_uint64()?;\n";
                    break;
                case BuiltinType::builtin_type_t::kInt8Type:
                    code << "                    let " << paramName << " = " << codecParamName << ".read_int8()?;\n";
                    break;
                case BuiltinType::builtin_type_t::kInt16Type:
                    code << "                    let " << paramName << " = " << codecParamName << ".read_int16()?;\n";
                    break;
                case BuiltinType::builtin_type_t::kInt32Type:
                    code << "                    let " << paramName << " = " << codecParamName << ".read_int32()?;\n";
                    break;
                case BuiltinType::builtin_type_t::kInt64Type:
                    code << "                    let " << paramName << " = " << codecParamName << ".read_int64()?;\n";
                    break;
                case BuiltinType::builtin_type_t::kFloatType:
                    code << "                    let " << paramName << " = " << codecParamName << ".read_float()?;\n";
                    break;
                case BuiltinType::builtin_type_t::kDoubleType:
                    code << "                    let " << paramName << " = " << codecParamName << ".read_double()?;\n";
                    break;
                case BuiltinType::builtin_type_t::kBoolType:
                    code << "                    let " << paramName << " = " << codecParamName << ".read_bool()?;\n";
                    break;
                case BuiltinType::builtin_type_t::kBinaryType:
                    code << "                    let " << paramName << " = " << codecParamName << ".read_binary()?;\n";
                    break;
                default:
                    code << "                    let " << paramName << " = Default::default(); // TODO: Handle "
                         << builtinType->getName() << "\n";
        ...
        {
            code << "                    let " << paramName << " = " << generateTypeRead(paramType, codecParamName)
                 << "?;\n";
        }
        else
        {
            code << "                    let " << paramName << " = Default::default(); // TODO: Handle type "
...
std::string RustGenerator::generateServerResponseSerialization(Function *fn, const std::string &codecParamName)
{
    stringstream code;
    ...
    if (hasReturnValue && hasOutParams)
    {
        ...
        {
            code << ", " << outParam.name; // Use actual parameter name
        }
        code << ") = response;\n";
        ...
        code << "                            " << generateTypeWrite(returnType, "return_value")
             << "; // Return value\n";
    }
    else if (hasOutParams)
    {
        ...
        else
        {
            ...
            for (size_t i = 0; i < outParams.size(); ++i)
            {
                if (i > 0)
                    code << ", ";
                code << outParams[i].name; // Use actual parameter name
            }
            code << ") = response;\n";
    ...
    else if (hasReturnValue)
    {
        ...
        code << "                            " << generateTypeWrite(returnType, "response") << "; // Return value\n";
    }
...
std::string RustGenerator::generateServerErrorResponseSerialization(Function *fn, const std::string &codecParamName)
{
    stringstream code;
    ...
    if (hasOutParams)
    {
        ...
        for (const auto &outParam : outParams)
        {
            code << "                            " << generateDefaultErrorWrite(outParam.type)
                 << "; // Default out parameter: " << outParam.name << "\n";
        }
    }
    ...
    if (hasReturnValue)
    {
        ...
        code << "                            " << generateDefaultErrorWrite(returnType) << "; // Error return value\n";
    }
...
std::string RustGenerator::generateDefaultErrorWrite(DataType *dataType)
{
    ...
    else if (dataType->getDataType() == DataType::data_type_t::kArrayType)
    {
        // For arrays, write default values for each element
        ArrayType *arrayType = dynamic_cast<ArrayType *>(dataType);
        DataType *elementType = arrayType->getElementType();
        uint32_t elementCount = arrayType->getElementCount();

        code << "for _ in 0.." << elementCount << " {\n";
        code << "        " << generateDefaultErrorWrite(elementType) << ";\n";
        code << "    }";
    }
...
std::string RustGenerator::generateTypeWrite(DataType *dataType, const std::string &variableName)
{
    stringstream code;
    ...
    {
        // For fixed-size arrays, write each element directly
        ArrayType *arrayType = dynamic_cast<ArrayType *>(dataType);
        DataType *elementType = arrayType->getElementType();
...
void RustGenerator::determineReturnTypes(Function *fn, bool hasOutParams, const std::vector<DataType *> &outParamTypes,
                                         std::string &rustReturnType, std::string &clientReturnType)
{
    DataType *returnType = fn->getReturnType();
    ...
    else if (hasOutParams)
    {
        ...
        {
            rustReturnType =
                " -> Result<" + getTypeString(outParamTypes[0]) + ", Box<dyn std::error::Error + Send + Sync>>";
    ...
    {
        // Function has only return value
        rustReturnType = " -> Result<" + getTypeString(returnType) + ", Box<dyn std::error::Error + Send + Sync>>";
        clientReturnType = rustReturnType;
    }
    else
    {
        ...
        rustReturnType = " -> Result<(), Box<dyn std::error::Error + Send + Sync>>";
        clientReturnType = rustReturnType;
    }
}

std::string RustGenerator::generateMethodCallParameters(Function *fn, const std::set<std::string> &lengthParams)
{
...
std::string RustGenerator::generateOnewayServerHandler(Function *fn, const std::string &codecParamName)
{
    ...
    serverHandlerCode << generateServerParameterDeserialization(fn, codecParamName);
...
std::string RustGenerator::generateRegularServerHandler(Function *fn, const std::string &codecParamName,
                                                        const std::string &sequenceParamName)
{
    ...
    std::string interfaceMethodId = interface->getName() + "Method::" + toPascalCase(fn->getName());
    ...
    serverHandlerCode << "                            let reply_info = "
                      ...
                      << interfaceServiceId << ".as_u8(), " << interfaceMethodId << ".as_u8(), " << sequenceParamName
                      << ");\n";
    ...
    serverHandlerCode << generateServerErrorResponseSerialization(fn, codecParamName);
...
std::string RustGenerator::generateClientMethodCode(Function *fn, std::vector<DataType *> &outParamTypes,
                                                    bool &hasOutParams)
{
    ...
    for (auto param : fn->getParameters().getMembers())
    {
        ...
        if (direction == param_direction_t::kInDirection || direction == param_direction_t::kInoutDirection)
        {
            ...
            if (clientLengthParams.find(param->getName()) == clientLengthParams.end())
            {
                inParams.push_back(param);
            }
        }
    ...
    else
    {
        ...
        clientMethodCode << generateClientResponseDeserialization(fn, outParamTypes, clientOutParamNames, hasOutParams);
    }
...
cpptempl::data_list RustGenerator::processGroupTypeAliases(Group *group)
{
    ...
    for (Symbol *symbol : group->getSymbols())
    {
        if (symbol->getSymbolType() == Symbol::symbol_type_t::kTypenameSymbol)
        {
            ...
            if (dataType && dataType->getDataType() == DataType::data_type_t::kAliasType)
            {
                AliasType *aliasType = dynamic_cast<AliasType *>(dataType);
                if (aliasType)
                {
...
cpptempl::data_list RustGenerator::processGroupInterfaces(Group *group)
{
    ...
    for (auto interface : group->getInterfaces())
    {
        ...
        interfaceInfo["serviceIdName"] = toUpperSnakeCase(interface->getName()) + "_SERVICE_ID";
        interfaceInfo["methodConstName"] = toUpperSnakeCase(interface->getName()) + "_METHOD";
...
cpptempl::data_list RustGenerator::processGroupEnums(Group *group)
{
    ...
    for (Symbol *symbol : group->getSymbols())
    {
        ...
        {
            DataType *dataType = dynamic_cast<DataType *>(symbol);
            if (dataType && dataType->getDataType() == DataType::data_type_t::kEnumType)
            {
                ...
                if (enumType)
                {
                    ...
                    enumInfo["writeCode"] = "codec.write_" + codecMethod + "(*self as " + reprType + ")?;";
...
cpptempl::data_list RustGenerator::processGroupStructs(Group *group)
{
    ...
    for (Symbol *symbol : group->getSymbols())
    {
        ...
        {
            DataType *dataType = dynamic_cast<DataType *>(symbol);
            if (dataType && dataType->getDataType() == DataType::data_type_t::kStructType)
            {
                ...
                if (structType)
                {
                    ...
                    for (auto member : structType->getMembers())
                    {
                        ...
                        memberInfo["writeCode"] = generateMemberWrite(member, "self." + memberName);
                        memberInfo["readCode"] = generateTypeRead(member->getDataType(), "codec");
                        setTemplateComments(member, memberInfo);
                        members.push_back(memberInfo);
                    }
...
std::string RustGenerator::generateBuiltinTypeWrite(BuiltinType *builtinType, const std::string &codecName,
                                                    const std::string &variableName, bool addErrorHandling)
{
    ...
    switch (builtinType->getBuiltinType())
    {
        ...
        case BuiltinType::builtin_type_t::kDoubleType:
            ...
            break;
        case BuiltinType::builtin_type_t::kStringType:
            operation = codecName + ".write_string(&" + variableName + ")" + errorSuffix;
            break;
        case BuiltinType::builtin_type_t::kBinaryType:
            operation = codecName + ".write_binary(&" + variableName + ")" + errorSuffix;
            break;
        default:
...
std::string RustGenerator::generateBuiltinTypeRead(BuiltinType *builtinType, const std::string &codecName)
{
    switch (builtinType->getBuiltinType())
    {
        case BuiltinType::builtin_type_t::kBoolType:
...
std::string RustGenerator::generateBuiltinTypeDefaultWrite(BuiltinType *builtinType, const std::string &codecName)
{
    switch (builtinType->getBuiltinType())
    {
        case BuiltinType::builtin_type_t::kBoolType:
        ...
        default:
            return codecName + ".write_uint8(u8::default())?"; // Fallback for unknown types
    }
}
